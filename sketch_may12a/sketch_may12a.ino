...
char instruction[12] = "FFFBLFBFRFBF";
int amount[12] = {
  115,
  85,
  199,
...
void loop() {
  ...
  motors.setSpeed(255); //Remember it's a 0-255 range
  switch(instruction[iterator])
  {
    case 'F':
      motors.setSpeed(amount[iterator]);
      motors.forward();
      break;
    case 'B':
      motors.setSpeed(amount[iterator]);
      ...
      break;
    case 'L':
      left_servo.write(amount[iterator]);
      right_servo.write(amount[iterator]);
      break;
    case 'R':
      left_servo.write(amount[iterator]);
      right_servo.write(amount[iterator]);
